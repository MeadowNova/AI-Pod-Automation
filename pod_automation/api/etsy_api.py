        ...
        try:
            ...
            if response.status_code >= 400:
                try:
                    ...
                    logger.error(f"Error response from Etsy API: {response.status_code} - {error_json}")
                except:
                    logger.error(f"Error response from Etsy API: {response.status_code} - {response.text}")
