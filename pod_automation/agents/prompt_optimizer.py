...
class PromptOptimizer:
    ...
    def generate_variations(self, base_prompt, count=3, **kwargs):
        """Generate variations of a base prompt.

        This is an alias for generate_prompt_variations to maintain compatibility with tests.

        Args:
            base_prompt (str): Base prompt to generate variations for
            count (int, optional): Number of variations to generate
            **kwargs: Additional arguments to pass to optimize_prompt

        Returns:
            list: List of optimized prompts (without negative prompts)
        """
        variations = self.generate_prompt_variations(base_prompt, num_variations=count, **kwargs)
        # Return only the optimized prompts, not the negative prompts
        return [prompt for prompt, _ in variations]
    ...
    def evaluate_prompt(self, prompt):
        ...
        """
        # Count the number of descriptive elements
        descriptive_elements = len(prompt.split(','))

        # Check for presence of important elements
        has_style = any(style.lower() in prompt.lower() for style in self.styles)
        has_artist = any(artist.lower() in prompt.lower() for artist in self.artists)
        has_modifier = any(modifier.lower() in prompt.lower() for modifier in self.modifiers)
        has_color = any(color.lower() in prompt.lower() for color in self.colors)
        has_technique = any(technique.lower() in prompt.lower() for technique in self.techniques)

        # Calculate base score
        base_score = min(descriptive_elements * 10, 50)

        # Add points for important elements
        element_score = sum([
            15 if has_style else 0,
            10 if has_artist else 0,
            10 if has_modifier else 0,
            10 if has_color else 0,
            5 if has_technique else 0
        ])

        # Calculate final score (capped at 100)
        score = min(base_score + element_score, 100)

        # Empty prompt gets a minimum score
        if not prompt.strip():
            score = 10.0

        # Convert to float to match test expectations
        return float(score)
...
def main():
    """Main function to test prompt optimizer."""
    ...
    optimizer.save_components()

    # Test prompt optimization
    base_prompt = "A cat wearing a t-shirt"
    optimized_prompt, negative_prompt = optimizer.optimize_prompt(base_prompt)
    ...
    variations = optimizer.generate_prompt_variations(base_prompt, num_variations=3)
    for i, (prompt, negative) in enumerate(variations, 1):
        print(f"\nVariation {i}:")
        print(f"Prompt: {prompt}")
        print(f"Negative: {negative}")

    # Test generating from keywords
    print("\nGenerating from keywords:")
    keywords = ["funny", "sleeping", "cartoon"]
    keyword_prompts = optimizer.optimize_from_keywords(keywords, num_prompts=2)
    for i, (prompt, negative) in enumerate(keyword_prompts, 1):
        print(f"\nKeyword Prompt {i}:")
        print(f"Prompt: {prompt}")
        print(f"Negative: {negative}")
